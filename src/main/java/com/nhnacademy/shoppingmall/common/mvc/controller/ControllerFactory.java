...
public class ControllerFactory {
    ...
    public void initialize(Set<Class<?>> c, ServletContext ctx) {
         ...
         */
        for (Class<?> clazz : c) {
            RequestMapping str = clazz.getDeclaredAnnotation(RequestMapping.class);
            if (str != null) {
                try {
                    String key = str.method() + "-" + String.join(",", str.value());
...
//        for(Class<?> clazz : c){
////            clazz => reflection을 통해서 클래스의 어노테이션을 가져올 수 있어.
//            clazz.getAnnotation(RequestMapping.class); => null / nullX
//            null이 아니라면?
//            -> controller이겠지!!
//            -> controller이라면 뭘해주야할까?
//            -> beanMap 넣어줘야돼
//                => key
//                    => method : clazz.getAnnotation(RequestMapping.class).method();
//                    => value : clazz.getAnnotation(RequestMapping.class).value();
//                        => [] 배열이어가지고 배열을 다 가져와야해
//                        => /index.do/main.do
//                => value
//                    => controller instance=> reflection이용해서 구현
...
//            => beanMap.put(key, value);
    ...
    public Object getController(String method, String path) {
        ...
        Object o = getBean(getKey(method, path));
        if (o == null) {
            throw new ControllerNotFoundException(getKey(method, path));
        }
        return o;
    }
